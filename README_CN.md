...
**本仓库当前版本：Kuroko v1.1.0**（变更记录见 §12）。
...
其中 `first_s/last_s/avg_sec` 是**纯信息性字段**，只用于丰富 `openai_auth` 轴的 reason 文案，
**不参与任何判定阈值**——已用新数据验证过，正常用户的鉴权请求也经常是几秒内的小突发，拿「突发」
本身当信号会误杀。`distinct_secs` 则是「域名并发异常」轴（§5.6）的唯一输入。
...
- 只有 **IPv4 目的地**参与横向/纵向端口扫描等绝大多数威胁判定；**域名目的地对这些轴天然免疫**
  → 这就是 REALITY 借用 SNI 不被误杀的结构性原因。例外只有两条：`openai_auth`（§5.4）与
  `domain_concurrency`（§5.6）——两者都只针对精确指定的域名/域名并发信号，且都已用真实数据
  验证过不会误伤 REALITY，不改变「域名整体对扫描类判定免疫」这个大原则。
...
6. **域名并发异常（2026-07-31 新增，唯一一条对*任意*域名生效的轴）**：域名目的地本来对本引擎
   所有扫描类威胁轴免疫（§4 原则），这条是唯一一条不限定域名名单的例外（§5.4 的 `openai_auth`
   虽然也对域名计数，但只认精确列举的那两组）。用时间戳字段算 `count / distinct_secs`
   （当日命中数 ÷ 命中的不同秒数）衡量「是否有多次命中砸进同一秒」——若同一域名当日命中数
   ≥`DOMAIN_CONCURRENCY_MIN_HITS`(20) 且该比值 ≥`DOMAIN_CONCURRENCY_RATIO_MIN`(2.0) → SUSPICIOUS。
   **已用真实数据校准**：对 29635 个 `count≥20` 且带时间戳的样本（含已知 REALITY 借用 SNI 的
   visa.cn / qualcomm.cn，以及不限定的全体域名）实测这个比值从未超过 1.833；REALITY 心跳本身该值
   恒为 1.0（低频、从不并发）。说明「同一秒内反复并发命中同一域名」在真实流量里是干净的统计异常，
   更像脚本/工具在多线程并发连接，而不是浏览器/正常客户端行为——不影响 REALITY/SNI 域名豁免的
   整体设计。
...
1. **扫描类判定不对域名计数** → REALITY/SNI 借用免疫。横向、纵向、P2P 这几条轴只认裸 IPv4，
   ...
   - 两条例外都不削弱这一点：`openai_auth`（§5.4）只对两组**精确列举**的 OpenAI 域名生效，
     借用的 SNI 域名不在名单里；`domain_concurrency`（§5.6）虽然对任意域名生效，但判的是
     「同一秒内并发命中」而不是「访问了什么域名」，实测 REALITY 心跳该比值恒为 1.0（低频、
     从不并发），离 2.0 的门槛很远。也就是说：**REALITY 借用 SNI 这件事本身，在任何一条轴上
     都不产生信号。**
...
| `DOMAIN_CONCURRENCY_MIN_HITS` / `DOMAIN_CONCURRENCY_RATIO_MIN` | 域名并发异常判定的最小命中数 / `count÷distinct_secs` 并发比值门槛 |
...
- 「域名并发异常」轴依赖上游的时间戳字段（2026-07-29 起才有）。**旧日期的数据没有这个字段，
